...
//  for(var m=0;m<100;m++){
//    let ttt=await device.get({'dps':m});
//    if(ttt)console.log(m,ttt);
//  }return;
  ...
  for(var i=0;i<Math.pow(2,bitsNumber);i++){
      ...
      newHsv[1]=sStateValues[sStateCounter];
  ...
  setColor(firstColor);
  console.log(chalk.bold.rgb(firstColorRGB[0], firstColorRGB[1], firstColorRGB[2])(firstColorHSV));
  await new Promise(r => setTimeout(r, timeoutDuration));
  sStateCounter=(sStateCounter+1)%2;
  ...
  for(var i=0;i<exfiltred.length;i++){
    ...
    if(exfiltred[i]=="0"){
      newHsv=Array.from(baseHsv);
      sStateCounter=1; //(sStateCounter+1)%2;
      newHsv[1]=sStateValues[sStateCounter];
      newRgb=hsvToRgb(newHsv[0],newHsv[1],newHsv[2]);
      rgb=Array.from(newRgb);
      hsv=Array.from(newHsv);
      ...
      await new Promise(r => setTimeout(r, timeoutDuration));
      newHsv=Array.from(baseHsv);
      sStateCounter=0; //(sStateCounter+1)%2;
      newHsv[1]=sStateValues[sStateCounter];
      //newHsv[0]=baseHsv[0];
      newRgb=hsvToRgb(newHsv[0],newHsv[1],newHsv[2]);
      rgb=Array.from(newRgb);
